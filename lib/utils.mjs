...
export function getInstantInfo(ms, ns = 0, zone, formatter) {
  const formatter = formatter || createFormatter(zone);
...
export function createFormatter(zone) {
  if (zone === 'SYSTEM') {
    return {
      nonGeographic: true,
      formatToParts: (date) => [
        { type: 'year', value: '' + date.getFullYear() },
        { type: 'literal', value: '-' },
        { type: 'month', value: '' + (date.getMonth() + 1) },
        { type: 'literal', value: '-' },
        { type: 'day', value: '' + date.getDate() },
        { type: 'literal', value: ' ' },
        { type: 'hour', value: '' + date.getHours() },
        { type: 'literal', value: ':' },
        { type: 'minute', value: '' + date.getMinutes() },
        { type: 'literal', value: ':' },
        { type: 'second', value: '' + date.getSeconds() }
      ]
    };
  }
